  ...
  methods: {
    ...
    keyDown(e: KeyboardEvent) {
      if (e.which === 38 || e.which === 40) {
        // If we have suggestions showing, navigate them
        if (this.currentSuggestions.length > 0) {
          e.preventDefault();
          if (e.which === 38) { // Up arrow
            this.suggestionIndex = Math.max(-1, this.suggestionIndex - 1);
          } else { // Down arrow
            this.suggestionIndex = Math.min(this.currentSuggestions.length - 1, this.suggestionIndex + 1);
          }
        } else {
          e.preventDefault();
        ...
        // If we have suggestions, autocomplete with the selected or first one
        if (this.currentSuggestions.length > 0) {
          const selectedIndex = this.suggestionIndex >= 0 ? this.suggestionIndex : 0;
          const suggestion = this.currentSuggestions[selectedIndex];
          if (suggestion) {
