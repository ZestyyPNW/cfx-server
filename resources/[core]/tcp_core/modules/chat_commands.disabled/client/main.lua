...
local function GetCharacterName()
    if currentCharacter and currentCharacter.firstname and currentCharacter.lastname then
        return tostring(currentCharacter.firstname) .. ' ' .. tostring(currentCharacter.lastname)
    end

    local ok, player = pcall(function()
        return exports["ND_Core"] and exports["ND_Core"]:getPlayer() or nil
    end)
...
AddEventHandler('scrp:chat:receiveProximityMessage', function(message, color)
    ...
    TriggerEvent('chat:addMessage', {
        multiline = false,
        template = '<div class="chat-message" style="color: rgb({0},{1},{2});">{3}</div>',
        args = { c[1], c[2], c[3], message }
    })
end)
...
print('^2[tcp_core]^7 Chat commands module loaded (client)')
