...
function UIMenu:GoUpOverflow()
    ...
    local currentIndex = self:CurrentSelection()
    local currentItem = self.Items[currentIndex]
    if not currentItem then
        return -- Prevent execution if current item doesn't exist
    end

    if currentIndex <= self.Pagination.Min + 1 then
        if currentIndex == 1 then
            ...
            local nextIndex = self:CurrentSelection()
            local nextItem = self.Items[nextIndex]
            if nextItem then
...
function UIMenu:GoUp()
    ...
    if nextItem then
        ...
        return -- Prevent further execution if next item doesn't exist
    end
...
function UIMenu:GoDownOverflow()
    ...
    if currentIndex >= self.Pagination.Max then
        if currentIndex == #self.Items then
            self.Pagination.Min = 0
            self.Pagination.Max = self.Pagination.Total + 1
            currentItem:Selected(false)
            self.ActiveItem = 1000 - (1000 % #self.Items)
