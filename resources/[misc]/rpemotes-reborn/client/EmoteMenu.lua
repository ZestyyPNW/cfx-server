...
local function extractPackNameFromComment(comment)
    comment = trim(comment):gsub("%s+", " ")
    if comment == "" then return nil end

    local by = comment:match("[Bb]y%s+(.+)$")
    local pack = trim(by or comment)
    pack = pack:gsub("%s*[Ee]xclusive.*$", "")
    pack = trim(pack)
    if pack == "" then return nil end
    return pack
end

local packByEmoteName = nil
local function getEmotePackMap()
    if packByEmoteName then return packByEmoteName end
    packByEmoteName = {}
    ...
    local function parseFile(path)
        ...
        for line in file:gmatch("[^\r\n]+") do
            local name, comment = line:match("^%s*%[%s*['\"]([^'\"]+)['\"]%s*%]%s*=%s*%{.-%-%-%s*(.+)$")
            if name and comment and comment ~= "" then
                local pack = extractPackNameFromComment(comment)
                if pack then
                    packByEmoteName[name] = pack
                end
            end
        end
    end

    parseFile("client/AnimationList.lua")
    parseFile("client/AnimationListCustom.lua")

    return packByEmoteName
end

local function buildPackMenus(parentMenu, rpTable, emoteType, itemPrefix, isPropMenu)
    local packMap = getEmotePackMap()

    local grouped = {}
    for name, data in pairs(rpTable) do
        local pack = packMap[name]
        if pack then
            if not grouped[pack] then grouped[pack] = {} end
            grouped[pack][#grouped[pack] + 1] = { name = name, data = data }
        end
    end

    local packNames = {}
    for packName in pairs(grouped) do
        packNames[#packNames + 1] = packName
    end
    table.sort(packNames)

    for i = 1, #packNames do
        local packName = packNames[i]
        local entries = grouped[packName]
        table.sort(entries, function(a, b) return a.name < b.name end)

        local packMenu = _menuPool:AddSubMenu(parentMenu, packName, "", true, true)
        local names = {}

        for j = 1, #entries do
            local entry = entries[j]
            local _, _, label = table.unpack(entry.data)
            label = label or entry.name

            local menuItem
            if isPropMenu and entry.data.AnimationOptions and entry.data.AnimationOptions.PropTextureVariations then
                menuItem = NativeUI.CreateListItem(label, entry.data.AnimationOptions.PropTextureVariations, 1, ("%s (%s)"):format(itemPrefix, entry.name))
            else
                menuItem = NativeUI.CreateItem(label, ("%s (%s)"):format(itemPrefix, entry.name))
            end

            packMenu:AddItem(menuItem)
            names[#names + 1] = entry.name
        end

        packMenu.OnIndexChange = function(menu, newindex)
            ClearPedTaskPreview()
            if names[newindex] then
                EmoteMenuStartClone(names[newindex], emoteType)
            end
        end

        packMenu.OnItemSelect = function(sender, item, index)
            if names[index] then
                EmoteMenuStart(names[index], emoteType)
            end
        end

        if isPropMenu then
            packMenu.OnListSelect = function(menu, item, itemIndex, listIndex)
                if names[itemIndex] then
                    EmoteMenuStart(names[itemIndex], emoteType, item:IndexToItem(listIndex).Value)
                end
            end
        end

        packMenu.OnMenuClosed = function(menu)
            ClearPedTaskPreview()
        end
    end
end
...
function AddEmoteMenu(menu)
    ...
    local keybindsItem = nil

    if Config.Search then
        searchItem = NativeUI.CreateItem(Translate('searchemotes'), "")
        submenu:AddItem(searchItem)
    end

    local allEmotesMenu = _menuPool:AddSubMenu(submenu, "All Emotes", "", true, true)
    local emotePacksMenu = _menuPool:AddSubMenu(submenu, "Emote Packs", "", true, true)
    local dancePacksMenu = _menuPool:AddSubMenu(submenu, "Dance Packs", "", true, true)
    local propPacksMenu = _menuPool:AddSubMenu(submenu, "Prop Packs", "", true, true)
    ...
    if not Config.SqlKeybinding then
        ...
        keybindsItem = NativeUI.CreateItem(Translate('keybinds'), Translate('keybindsinfo') .. " /emotebind [~y~num4-9~w~] [~g~emotename~w~]")
        submenu:AddItem(keybindsItem)
    end

    local allEmoteKeys = {}
    for a, b in PairsByKeys(RP.Emotes) do
        local x, y, z = table.unpack(b)
        allEmotesMenu:AddItem(NativeUI.CreateItem(z, "/e (" .. a .. ")"))
        table.insert(allEmoteKeys, a)
        if not Config.SqlKeybinding then
            favEmotes[a] = z
        end
    end

    buildPackMenus(emotePacksMenu, RP.Emotes, "emotes", "/e", false)
    buildPackMenus(dancePacksMenu, RP.Dances, "dances", "/e", false)
    buildPackMenus(propPacksMenu, RP.PropEmotes, "props", "/e", true)
    ...
    allEmotesMenu.OnIndexChange = function(menu, newindex)
        ClearPedTaskPreview()
        if allEmoteKeys[newindex] then
            EmoteMenuStartClone(allEmoteKeys[newindex], "emotes")
        end
    end

    allEmotesMenu.OnItemSelect = function(sender, item, index)
        if allEmoteKeys[index] then
            EmoteMenuStart(allEmoteKeys[index], "emotes")
        end
    end

    allEmotesMenu.OnMenuClosed = function(menu)
        ClearPedTaskPreview()
    end

    submenu.OnItemSelect = function(sender, item, index)
        if Config.Search and item == searchItem then
            EmoteMenuSearch(submenu)
        elseif not keybindsItem or item ~= keybindsItem then
            return
        end
    end
