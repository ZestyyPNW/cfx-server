...
local function GetCharacterName(source)
    ...
    TriggerEvent('scrp:characters:getCharacter', source, function(character)
        charData = character
    end)
    
    if charData and charData.firstname and charData.lastname then
        return charData.firstname .. ' ' .. charData.lastname
    end
...
print('^2[SCRP Core]^7 Roleplay chat commands server loaded')
